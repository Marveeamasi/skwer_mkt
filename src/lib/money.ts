...
export function kobo(value: number): Kobo {
  if (!Number.isSafeInteger(value) || value < 0) throw new RangeError("Money must be a non-negative safe integer in kobo");
...
export function roundUp(value: number, increment: number): Kobo {
  if (!Number.isSafeInteger(increment) || increment <= 0) throw new RangeError("Rounding increment must be positive");

...
export function estimatePaystackFee(gross: number, config: FeeConfig): Kobo {
  const percentage = Math.ceil((gross * config.paystackPercentBasisPoints) / 10_000);
  const fixed = gross >= config.paystackFixedThresholdKobo ? config.paystackFixedKobo : 0;
...
export function platformFee(sellerMinimum: number, config: FeeConfig): Kobo {
  const calculated = Math.ceil((sellerMinimum * config.platformPercentBasisPoints) / 10_000) + config.platformFlatKobo;
  return kobo(Math.min(Math.max(calculated, config.platformMinKobo), config.platformMaxKobo ?? Number.MAX_SAFE_INTEGER));
}

export function calculatePublicPrice(sellerMinimum: number, reward: number, config: FeeConfig): PricingResult {
  kobo(sellerMinimum); kobo(reward); kobo(config.safetyBufferKobo);
  const platform = platformFee(sellerMinimum, config);
  const requiredAfterProcessor = sellerMinimum + reward + platform + config.safetyBufferKobo;
  ...
  while (gross - fee < requiredAfterProcessor) {
    ...
    if (++iterations > 100_000) throw new Error("Unable to converge on a valid public price");
  }
  return { publicPriceKobo: gross, sellerTakeHomeKobo: kobo(sellerMinimum), rewardFundingKobo: kobo(reward), estimatedPaystackFeeKobo: fee, platformRevenueKobo: platform, safetyBufferKobo: kobo(config.safetyBufferKobo) };
}

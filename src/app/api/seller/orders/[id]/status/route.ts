...
import { qualifyReferralReward } from "@/server/growth";
...
export async function POST(
  ...
  try {
    ...
    const admin = createAdminClient(),
      { error } = await admin.from("orders").update({ status }).eq("id", id);
    ...
    await admin
      ...
      .insert({
        ...
        from_status: owned.status,
        to_status: status,
        actor_type: "seller",
        actor_id: userId,
        public_message: `Order ${status.replaceAll("_", " ")}`,
      });
    if (status === "delivered" || status === "picked_up")
      await qualifyReferralReward(id);
    await audit({
      actorId: userId,

...
export async function POST(
  ...
  try {
    ...
    const { data: order } = await session
      .from("orders")
      .select("id,status,total_due_kobo,total_paid_kobo")
      .eq("id", id)
      .single();
    if (!order)
    ...
    if (
      ...
      return NextResponse.json(
        { error: "A balance link is not available for this order status" },
        { status: 409 },
      );
    ...
    await audit({
      actorId: userId,
      action: "order.balance_link_created",
      resourceType: "order",
      resourceId: id,
      after: {
    ...
    return NextResponse.json({
      url: `${publicConfig.NEXT_PUBLIC_APP_URL}/balance/${token.token}`,
      expiresAt: expiresAt.toISOString(),
      outstandingKobo: outstanding,
    });
  } catch (error) {

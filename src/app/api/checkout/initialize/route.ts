...
function safeMessage(error: unknown) {
  ...
  const rewardMessages: Record<string, string> = {
    reward_not_found: "That reward was not found.",
    reward_unavailable: "That reward is no longer available.",
    reward_wrong_seller: "That reward belongs to another seller.",
    reward_wrong_owner: "Use the phone number that earned this reward.",
    reward_expired: "That reward has expired.",
    reward_referral_conflict: "A reward cannot be combined with a referral.",
    reward_already_applied: "A reward is already applied to this order.",
  };
  return (
    rewardMessages[message] ??
    (message === "insufficient_stock"
      ? "That option has just sold out."
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!featureFlags.payments) {
      if (process.env.NODE_ENV !== "production")
        return NextResponse.json({
          authorizationUrl: `/payment/callback?reference=DEMO-${nanoid(10)}&demo=1`,
        });
      return NextResponse.json(
        { error: "Secure payments are not enabled for this pilot yet." },
        { status: 503 },
      );
    }
    ...
    const { data: campaign, error: campaignError } = await db
      .from("campaigns")
      .select(
        "*,product:products(*),seller:seller_businesses(*),payment_account:seller_payment_accounts(*)",
      )
      .eq("short_code", input.campaignCode)
      .eq("status", "active")
      .single();
    ...
    if (code && featureFlags.rewards && !input.rewardCode) {
      const { data: link } = await db
        .from("referral_links")
        .select(
          "*,referrer:customers!referral_links_referrer_customer_id_fkey(normalized_phone,email)",
        )
        .eq("public_code", code)
        ...
        .maybeSingle();
      const referrer = Array.isArray(link?.referrer)
        ? link.referrer[0]
        : link?.referrer;
      if (
        link &&
        referrer &&
        isDistinctReferral(
          { phone: referrer.normalized_phone, email: referrer.email },
    ...
    const snapshot = {
      ...campaign.pricing_snapshot,
      paymentMode: campaign.payment_mode,
      totalOrderValueKobo: originalTotal,
    };
    const { data: order, error: orderError } = await db
      ...
      .insert({
        ...
        item_subtotal_kobo: originalTotal,
        total_due_kobo: originalTotal,
        seller_target_kobo:
          Number(campaign.seller_minimum_take_home_kobo) * input.quantity,
        platform_target_kobo:
          Number(campaign.pricing_snapshot.platformRevenueKobo ?? 0) *
          input.quantity,
        reward_funding_kobo: referralId
          ? Number(campaign.referral_reward_kobo) * input.quantity
          : 0,
        paystack_fee_estimate_kobo:
          Number(campaign.pricing_snapshot.estimatedPaystackFeeKobo ?? 0) *
          input.quantity,
        buyer_note: input.buyerNote || null,
        referred_by_referral_id: referralId,
    ...
    const { error: itemError } = await db.from("order_items").insert({
      ...
      product_name_snapshot: campaign.product.name,
      variant_snapshot: {
        option1: variant.option_1_value,
        option2: variant.option_2_value,
      },
      quantity: input.quantity,
      unit_public_price_kobo: unitPrice,
      unit_seller_target_kobo: Number(campaign.seller_minimum_take_home_kobo),
    });
    ...
    if (campaign.stock_mode === "fixed") {
      const { error: reserveError } = await db.rpc("reserve_variant", {
        ...
        p_expires: new Date(
          Date.now() + Number(campaign.reservation_minutes) * 60000,
        ).toISOString(),
      });
    ...
    if (input.rewardCode && featureFlags.rewards) {
      const { data: amount, error: rewardError } = await db.rpc(
        "redeem_reward_atomic",
        {
          p_order: order.id,
          p_reward_hash: hashToken(input.rewardCode),
    ...
    const paymentMode = campaign.payment_mode as
      | "full"
      | "fixed_deposit"
      | "percentage_deposit";
    const dueNow = depositDue(
      totalDue,
      paymentMode,
      campaign.deposit_value == null
        ? undefined
        : Number(campaign.deposit_value),
    );
    ...
    const account = Array.isArray(campaign.payment_account)
      ? campaign.payment_account[0]
      : campaign.payment_account;
    const paystack = await initializePaystack({
      email: input.email,
      amount: dueNow,
      reference: paymentReference,
      callbackUrl: `${publicConfig.NEXT_PUBLIC_APP_URL}/payment/callback`,
      subaccount: featureFlags.splitPayments
        ? account?.subaccount_code
        : undefined,
      transactionCharge: Number(
        campaign.pricing_snapshot.platformRevenueKobo ?? 0,
      ),
      metadata: {
    ...
    if (createdOrderId) {
      ...
      try {
        const { error: cleanupError } = await createAdminClient().rpc(
          "compensate_checkout_failure",
          { p_order: createdOrderId, p_reason: message },
        );
        if (cleanupError) console.error("checkout-compensation", cleanupError);
      } catch (cleanupError) {
        console.error("checkout-compensation", cleanupError);
      }
    }

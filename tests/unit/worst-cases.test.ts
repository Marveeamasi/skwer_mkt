...
describe("money boundary safety", () => {
  ...
  ])("rejects invalid kobo: %s", (value) =>
    expect(() => kobo(value)).toThrow(RangeError),
  );
  it("rejects invalid rounding increments", () =>
    expect(() => roundUp(100, 0)).toThrow(RangeError));
  it("does not let a fixed deposit exceed the order total", () =>
    expect(depositDue(10_000, "fixed_deposit", 20_000)).toBe(10_000));
  it.each([0, -1, 10_001])("rejects unsafe deposit percentages: %s", (value) =>
    expect(() => depositDue(10_000, "percentage_deposit", value)).toThrow(
      RangeError,
    ),
  );
  it("rounds fractional percentage deposits upward", () =>
    expect(depositDue(101, "percentage_deposit", 5000)).toBe(51));
});
...
describe("order state-machine worst cases", () => {
  it("rejects unknown states, self transitions and terminal-state exits", () => {
    ...
    for (const state of Object.keys(allowedOrderTransitions))
      expect(canTransition(state, state)).toBe(false);
    for (const terminal of ["refunded", "cancelled", "expired"])
      expect(allowedOrderTransitions[terminal]).toEqual([]);
  });
...
describe("reward and referral abuse prevention", () => {
  ...
  const input = {
    ...
    orderSubtotalKobo: 3_000,
    hasReferral: false,
  };
  it("caps credit at the order subtotal", () =>
    expect(validateReward({ reward: base, ...input })).toEqual({
      valid: true,
      amountKobo: 3_000,
    }));
  it("rejects wrong seller, wrong phone, expired, unavailable, and stacked credit", () => {
    expect(
      validateReward({ reward: { ...base, sellerId: "seller-b" }, ...input })
        .valid,
    ).toBe(false);
    expect(
      validateReward({ reward: base, ...input, buyerPhone: "2348099999999" })
        .valid,
    ).toBe(false);
    expect(
      validateReward({ reward: { ...base, expiresAt: new Date(0) }, ...input })
        .valid,
    ).toBe(false);
    expect(
      validateReward({ reward: { ...base, status: "redeemed" }, ...input })
        .valid,
    ).toBe(false);
    expect(
      validateReward({ reward: base, ...input, hasReferral: true }).valid,
    ).toBe(false);
  });
  it("treats phone or case-insensitive email reuse as self-referral", () => {
    expect(
      isDistinctReferral(
        { phone: "1", email: "one@example.com" },
        { phone: "1", email: "two@example.com" },
      ),
    ).toBe(false);
    expect(
      isDistinctReferral(
        { phone: "1", email: "ONE@example.com" },
        { phone: "2", email: "one@example.com" },
      ),
    ).toBe(false);
  });
  it("drops expired attribution and honours explicit removal", () => {
    const expired = { code: "OLD", expiresAt: new Date(0) },
      valid = { code: "NEW", expiresAt: new Date(Date.now() + 60_000) };
    expect(
      chooseAttribution({
        existing: expired,
        incoming: null,
...
describe("untrusted input safety", () => {
  it.each([
    ...
    "080312345678",
    "not-a-phone",
  ])("rejects invalid Nigerian phone: %s", (phone) =>
    expect(() => normalizeNigerianPhone(phone)).toThrow(),
  );

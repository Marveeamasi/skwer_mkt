...
describe("webhook", () => {
  it("uses raw body", () => {
    const b = '{"event":"charge.success"}',
      s = "secret",
      h = createHmac("sha512", s).update(b).digest("hex");
    expect(verifyPaystackSignature(b, h, s)).toBe(true);
    expect(verifyPaystackSignature(b + " ", h, s)).toBe(false);
  });
});
